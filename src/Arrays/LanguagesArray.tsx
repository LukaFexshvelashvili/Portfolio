import HTML from "../images/Languages/HTML.webp";
import CSS from "../images/Languages/CSS.webp";
import BOOTSTRAP from "../images/Languages/BOOTSTRAP.webp";
import SCSS from "../images/Languages/SCSS.webp";
import JAVASCRIPT from "../images/Languages/JAVASCRIPT.webp";
import JQUERY from "../images/Languages/JQUERY.webp";
import TYPESCRIPT from "../images/Languages/TYPESCRIPT.webp";
import REACT from "../images/Languages/REACT.webp";
import VITE from "../images/Languages/VITE.webp";
import PHP from "../images/Languages/PHP.webp";
import MYSQL from "../images/Languages/MYSQL.webp";
import FIGMA from "../images/Languages/FIGMA.webp";
import TAILWIND from "../images/Languages/TAILWIND.webp";
import NODEJS from "../images/Languages/NODEJS.webp";

export const LanguagesArray = [
  { getId: 1, image: HTML, name: "HTML" },
  { getId: 2, image: CSS, name: "CSS" },
  { getId: 3, image: BOOTSTRAP, name: "BOOTSTRAP" },
  { getId: 4, image: SCSS, name: "SCSS" },
  { getId: 5, image: TAILWIND, name: "TAILWIND" },
  { getId: 6, image: JAVASCRIPT, name: "JAVASCRIPT" },
  { getId: 7, image: JQUERY, name: "JQUERY" },
  { getId: 8, image: TYPESCRIPT, name: "TYPESCRIPT" },
  { getId: 9, image: REACT, name: "REACT" },
  { getId: 10, image: NODEJS, name: "NODEJS" },
  { getId: 11, image: VITE, name: "VITE" },
  { getId: 12, image: PHP, name: "PHP" },
  { getId: 13, image: MYSQL, name: "MYSQL" },
  { getId: 14, image: FIGMA, name: "FIGMA" },
];
export const LanguagesNArray = [
  { getId: 1, styler: "HTMLDot", name: "HTML" },
  { getId: 2, styler: "CSSDot", name: "CSS" },
  { getId: 3, styler: "BOOTSTRAPDot", name: "BOOTSTRAP" },
  { getId: 4, styler: "SCSSDot", name: "SCSS" },
  { getId: 5, styler: "TAILWINDDot", name: "TAILWIND" },
  { getId: 6, styler: "JAVASCRIPTDot", name: "JAVASCRIPT" },
  { getId: 7, styler: "JQUERYDot", name: "JQUERY" },
  { getId: 8, styler: "TYPESCRIPTDot", name: "TYPESCRIPT" },
  { getId: 9, styler: "REACTDot", name: "REACT" },
  { getId: 10, styler: "NODEJSDot", name: "NODEJS" },
  { getId: 11, styler: "VITEDot", name: "VITE" },
  { getId: 12, styler: "PHPDot", name: "PHP" },
  { getId: 13, styler: "MYSQLDot", name: "MYSQL" },
  { getId: 14, styler: "FIGMADot", name: "FIGMA" },
];
